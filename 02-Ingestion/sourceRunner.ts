...
import { fetchBerwaldhallenTixlyEvents } from './sources/berwaldhallenTixly';
...
// Map event format to RawEventInput
...
export async function runEndtoendSources(): Promise<SourceResult[]> {
  ...
  // Berwaldhallen Tixly API - discovered via networkInspector
  try {
    const events = await fetchBerwaldhallenTixlyEvents();
    const queued = await queueEvents(events, 'berwaldhallen-tixly');
    results.push({
      source: 'berwaldhallen-tixly',
      eventsQueued: queued,
      eventsFound: events.length,
      errors: [],
      status: queued > 0 ? 'success' : 'fail',
...
// Run Kulturhuset source standalone
