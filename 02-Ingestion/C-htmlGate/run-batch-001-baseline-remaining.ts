/**
 * C-htmlGate Batch 001 - Remaining 5 sources
 * Sources: lulea-tekniska-universitet, moderna-museet, naturhistoriska-riksmuseet, orebro-sk, polismuseet
 */
...
async function main() {
  console.log('Starting Batch 001 baseline run (remaining 5 sources)...');
  ...
  writeFileSync(`${reportDir}/baseline-results-remaining-5.jsonl`, JSON.stringify(results, null, 2));
  console.log(`\nResults saved to ${reportDir}/baseline-results-remaining-5.jsonl`);
  
  // Summary
  const successCount = results.filter(r => r.success).length;
  const eventsTotal = results.reduce((sum, r) => sum + (r.extract?.eventsFound || 0), 0);
  
  console.log('\n=== REMAINING 5 SOURCES SUMMARY ===');
