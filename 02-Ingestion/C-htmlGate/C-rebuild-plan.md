...
## 12. C4-AI / lärloopen (UPPDATERAD MED failCategory — EXPERIMENTAL)
...
### 12.1 failCategory (EXPERIMENTAL — NOT CANONICAL YET)

Efter C4-AI-analys tilldelas varje fail-source en **failCategory**:

|| failCategory | Betydelse |
|--------------|-----------|
| `ENTRY_PAGE_NO_EVENTS` | **(2026-04-14 NY)** Entry page saknar events — C4 ska först försöka human-like discovery |
| `NEEDS_SUBPAGE_DISCOVERY` | Events finns på subpages, inte på entry |
| `LIKELY_JS_RENDER` | Sidan kräver JavaScript-rendering |
| `EXTRACTION_PATTERN_MISMATCH` | HTML-struktur matchar inte förväntat mönster |
| `LOW_VALUE_SOURCE` | Lågt värde som event källa |
| `UNKNOWN` | Oklassificerad |

### 12.2 C4-AI routing (EXPERIMENTAL — NOT CANONICAL YET)

Baserat på failCategory + confidence föreslår C4-AI routing:

|| failCategory | Confidence | Routing |
|---------------|------------|---------|
| `LIKELY_JS_RENDER` | ≥ 0.70 | → D (D-renderGate) |
| `ENTRY_PAGE_NO_EVENTS` | hög | → C1 (retry med upptäckta paths) |
| `NEEDS_SUBPAGE_DISCOVERY` | hög | → C1 (retry med subpage) |
| `EXTRACTION_PATTERN_MISMATCH` | - | → C1/D/discard |
| `LOW_VALUE_SOURCE` | - | → discard (manual-review) |
| `UNKNOWN` | - | → discard |

**Viktigt:** C4-AI routing är EXPERIMENTAL och INTE CANONICAL YET. routing-beslut baseras på failCategory men kopplingen suggestedRules → C1/C2/C3-ändring är inte verifierad.

### 12.3 System-status (UPPDATERAD 2026-04-12)

```
System: experimental
Canonical status: not canonical yet
Verification level: partially verified
C4-AI: IMPLEMENTED_EARLY_VERSION (failCategory + routing + rule persistence)
Batch-13: 14 sources × 3 rounds completed
Batch-14: 10 sources × 1 round completed (pools exhausted after routing decisions)
FailCategory: ENTRY_PAGE_NO_EVENTS (2026-04-14 NY), NEEDS_SUBPAGE_DISCOVERY, LIKELY_JS_RENDER, EXTRACTION_PATTERN_MISMATCH, LOW_VALUE_SOURCE, no_viable_path_found (2026-04-14 NY), robots_or_policy_blocked (2026-04-14 NY), likely_js_render_required (2026-04-14 NY), ambiguous_multiple_paths (2026-04-14 NY), insufficient_html_signal (2026-04-14 NY), UNKNOWN
Routing targets: C1 (retry), D (render-gateway), B, discard (manual-review)

LEARNING LOOP STATUS: partially learning

Rule persistence (batch-14):
- c4-derived-rules.jsonl: 6 rules saved
- Rule types: per-source routing hints (NOT general C1/C2/C3 changes)
- Rules loaded via loadAllDerivedRules()
- Routing changed for 5 of 9 fail-sources (B/D routing in round 1)

Two separate rule stores:
- c4-derived-rules.jsonl: per-source routing hints (batch-14: 6 entries)
- improvements-bank.jsonl: general C1/C2/C3 improvements (8 entries, last updated 2026-04-06)
```

### 12.4 C4-AI Integration Status (UPPDATERAD 2026-04-12)

**C4-AI genererar:**
- failCategory per fail-source ✓
- nextQueue (routing suggestion) ✓
- suggestedRules per fail-source ✓
- c4-ai-analysis-round-X.md produceras ✓
- c4-derived-rules.jsonl sparas ✓

**C4-AI påverkar routing:**
- suggestedRules sparas permanent till c4-derived-rules.jsonl ✓
- Rules lastas vid körning via loadAllDerivedRules() ✓
- 5 sources i batch-14 fick ändrad routing (B eller D) ✓
- Routing-ändring sker INOM samma batch, inte mellan batchar ✓

**C4-AI påverkar INTE C1/C2/C3:**
- c4-derived-rules.jsonl innehåller per-source routing hints, inte generella förbättringar ✗
- Inga C1/C2/C3-ändringar baserade på C4-AI:s suggestedRules ✗
- c4-ai-learnings.md fortfarande "IMPLEMENTATION GAP" ✗
- improvements-bank.jsonl uppdaterades INTE av batch-14 ✗
- Samma fail-set omkört EFTER ändring: EJ VERIFIERAT ✗

**Status:** `partially learning` (updated from `analysis only`)

**Krävs för att nå `operational learning loop`:**
1. Generella regler baserade på C4-AI:s observed patterns måste testas på C1/C2/C3
2. batch-15: omkörning av samma fail-set EFTER generell ändring
3. Före/efter-jämförelse som demonstrerar förbättrat utfall
...
### <implemented_early_version>
- **C4-AI** — IMPLEMENTED_EARLY_VERSION. Kod och integration finns i run-dynamic-pool.ts. Anropar runC4Analysis() efter varje round. Skriver c4-ai-analysis-round-X.md. Ger: likelyCategory, nextQueue, improvementSignals, suggestedRules, confidenceBreakdown. Pilotfall: bk-hacken, blekholmen. **Gap:** suggestedRules kopplade inte ännu till C1/C2/C3. C4-AI route:ar sources men påverkar inte C1/C2/C3 förändring.
</implemented_early_version>
...
**Mognadsgrad:** Experimental — rounds 1-3 körning verifierad (2026-04-11). Resume verifierad (2026-04-11). C4-AI är IMPLEMENTED_EARLY_VERSION (pilotfall: bk-hacken, blekholmen).

**Får INTE beskrivas som "canonical" förrän:** C4-AI demonstrerad i verklig körning med förbättring av fail-mängd, och suggestedRules kopplade till C1/C2/C3.
