...
**Status: GÄLLER FRÅN OCH MED 2026-04-11 — ERSÄTTER ALLA TIDIGARE FORMULERINGAR OM STATISK BATCH**

> **AUKTORITATIV KÄLLA** för 123-logiken.
> **DENNA FIL DEFINIERAR DEN CANONICAL MODELLEN FÖR C-TESTRIGGEN FRÅN OCH MED 2026-04-11.**
> Den ersätter alla tidigare formuleringar om "samma 10 källor genom tre rundor".
...
- Aktiva testpoolen och dess fail-mängder (sources som lämnat poolen via exitvillkor)
...
1. Köra C-batchmaker för att skapa/fylla aktiv testpool från `postB-preC`
...
## NY CANONICAL MODELL: DYNAMISK TESTPOOL (GÄLLER FRÅN 2026-04-11)

**DENNA MODELL ERSÄTTER ALLA TIDIGARE FORMULERINGAR OM "STATISK 10-KÄLLORS BATCH GENOM TRE RUNDOR".**
...
Ersätter alla tidigare restkö-namn **som fysisk pipeline-kö** (inklusive `postTestC-Fail`, `postTestC-Fail-round1/2/3`).

**Round-1/2/3 som spårnings- och rapporteringsbegrepp ersätts inte** — varje source har fortfarande `roundNumber: 1|2|3` och varje runda dokumenteras separat.

### C4-AI-roll
C4-AI analyserar fel och lärdomar. AI får **inte** vara dold fallback som "räddar" individuella fail-fall. Network Error ≠ Automatic Routing Signal.
...
## STEG 0c: Inflöde till C-testpoolen (UPPDATERAD 2026-04-11)

**DENNA SEKTION ERSÄTTER ALLA TIDIGARE FORMULERINGAR OM "INCOMING C-CANDIDATE QUEUE".**

### Endast en inkommande pool för refill: `postB-preC`

**Den enda inkommande poolen för påfyllning av den aktiva C-testpoolen är `postB-preC`.**

Batchmaker väljer nya sources för refill enbart från `postB-preC`. Inga andra köer eller källistor får användas för att fylla på testpoolen.

### Tidigare formulering är obsolet

Den gamla formuleringen om "incoming C-candidate queue = batch-state.jsonl + sources_status.jsonl" är **obsolet**.

Den nya modellen definierar:
- **Aktiv testpool**: max 10 C-sources som aktivt genomgår testprocessen
- **Refill-pool**: `postB-preC` — enda källan för nya sources till testpoolen
- **Exit-köer**: `postTestC-UI`, `postTestC-A`, `postTestC-B`, `postTestC-D`, `postTestC-manual-review`

### Filterregler för eligible C-källor (refill)

En källa i `postB-preC` är eligible för refill om:
- `triageResult = "html_candidate"`
- `status ∈ {"fail", "triage_required"}`
- `pendingNextTool ≠ "D-renderGate"`
- `triageRecommendedPath ≠ "manual_review"`
- source har inte tidigare deltagit i denna testkörning (ingen tidigare `roundsParticipated > 0`)
...
### Steg 1b: C-Batch-Loop Mode — Dynamisk testpool (GÄLLER FRÅN 2026-04-11)

**DENNA MODELL ERSÄTTER ALLA TIDIGARE FORMULERINGAR OM "SAMMA 10 KÄLLOR GENOM TRE RUNDOR".**

**STYRANDE VILLKOR:**
- `batch-state.jsonl` finns OCH `status ∈ {pending, idle, testing}`
- Då är Steg 1b AUTOMATISKT aktivt
- `current-task.md` IGNORERAS i detta fall
- `handoff.md` IGNORERAS i detta fall

**Dynamisk testpool — Processöversikt (Steg 0–7):**

```
STEG 0: Ingestion-Routing ✓ (körs alltid först)
  → Sortera källor: A/B/C/D/E

STEG 1: Skapa initial aktiv testpool
  → Batchmaker väljer 10 eligible C-källor från postB-preC
  → poolRoundNumber = 1

STEG 2: Kör C1→C2→C3 (utan AI) på alla kvarvarande aktiva sources
  → Varje source får resultat
  → Varje source hamnar i exakt en utfallskö

STEG 3: Exit-kontroll (per source)
  → events hittades → postTestC-UI (source lämnar poolen)
  → A/B/D-signal (hög konfidens) → postTestC-A/B/D (source lämnar poolen)
  → 3 rundor utan events → postTestC-manual-review (source lämnar poolen)
  → övriga fail → kvar i poolen (inför nästa runda)

STEG 4: Refill (mellan rundor, endast om pool < 10)
  → Batchmaker fyller på med nya eligible C-källor från postB-preC
  → Sources som redan lämnat poolen får inte återkomma
  → Refill får endast ske mellan rundor, aldrig mitt i en runda

STEG 5: C4-AI-analys
  → Analysera fail-mängden från rondan
  → Klassificera nätverksfel enligt felmodellen
  → Föreslå små generella förbättringar (ej site-specifika)

STEG 6: Nästa runda (om kvarvarande aktiva sources finns, max 3 per source)
  → poolRoundNumber += 1
  → Kör på kvarvarande + nytillagda sources
  → Upprepa Steg 2–5

STEG 7: Avslut
  → När alla aktiva sources lämnat poolen ELLER alla nått 3 rundor
  → Skriv slutrapport på alla fyra nivåer
  → Uppdatera batch-state.jsonl: status="completed"
  → AVSLUTA
```

**Per-source-regel (PINSKILD):**
- Varje source har eget `roundsParticipated`
- Source lämnar poolen så snart exitvillkor uppnås
- Source som lämnat poolen får inte återkomma i samma testkörning
- Max 3 rundor per source

**Poolregel:**
- Poolen är dynamisk mellan rundor men fast under själva rundan
- `poolRoundNumber` beskriver vilken runda testpoolen genomgår
- Batchbegreppet har justerats: poolnivå ersätter det gamla "samma 10 genom tre rundor"

**StopReason (poolnivå):**
- `pool-exhausted`: alla aktiva sources har lämnat poolen
- `max-rounds-reached`: alla kvarvarande sources har nått 3 rundor
- `no-sources-available`: inga eligible sources finns i postB-preC för refill
...
### Dynamisk testpool: Refill och Exit (GÄLLER FRÅN 2026-04-11)

**DENNA MODELL ERSÄTTER ALLA TIDIGARE FORMULERINGAR OM "STATISK 10-KÄLLORS BATCH GENOM TRE RUNDOR".**

#### Aktiv testpool vs refill-pool

Den enda inkommande poolen för refill är **`postB-preC`**.

Batchmaker får endast fylla på testpoolen från **`postB-preC`** — inte från "alla sources" eller godtyckliga urval.

#### Refill-regler

- **Refill får endast ske mellan rundor**, aldrig mitt i en runda
- Refill görs av Batchmaker när testpoolen har färre än 10 kvarvarande sources efter en runda
- Refill-urval: välj nya eligible C-källor från `postB-preC`
- Mål: återställa poolen till 10 inför nästa runda, om tillräckligt många eligible sources finns
- En source som redan lämnat poolen (via exitvillkor) får **inte** återkomma i senare rundor

#### Exitvillkor (per source)
...
#### `postTestC-manual-review` ersätter alla tidigare restkö-namn som fysisk pipeline-kö

`postTestC-manual-review` är det **explicita namnet** på restköen för sources som:
- har deltagit i 3 rundor
- utan events
- utan fastställd A/B/D-routing

**Ersätter:** `postTestC-Fail`, `postTestC-Fail-round3` (som fysisk slutlig kö).

**Men round-1/2/3 som spårnings-, rapporterings- och erfarenhetsbanksbegrepp ersätts inte.**
- `roundNumber: 1|2|3` är obligatoriskt fält i varje källrapport och batchrapport
- Lag 3-rundrapport skapas för varje round 1, 2 och 3
- Före/efter-jämförelse görs per runda
- Erfarenhetsbanken byggs på round-specifik data
- C4-AI-analysen görs per runda

#### Exempel: Poolens livscykel

```
Runda 1: 10 sources → 3 lämnar (2 events, 1 D-signal) → 7 kvarvarande
Refill: Batchmaker fyller på med 3 nya från postB-preC → 10 aktiva
Runda 2: 10 sources → 2 lämnar (1 A-signal, 1 fail efter 3:e rondan) → 8 kvarvarande
Refill: Batchmaker fyller på med 2 nya → 10 aktiva
Runda 3: 10 sources → alla lämnar (events/A/B/D/manual-review)
→ Poolen tom → testkörning klar
```

#### 123-loopens flöde i dynamisk poolmodell

```
Runda N:
  1. Kör C1→C2→C3 på alla kvarvarande aktiva sources
  2. Varje source hamnar i exakt en utfallskö (enligt exitvillkor)
  3. Poolen krymper

Mellan rundor:
  4. Batchmaker gör refill från postB-preC (om pool < 10)
  5. C4-AI analyserar fail-mängden från rondan
  6. Eventuella små generella förbättringar görs

Nästa runda:
  7. Kör på kvarvarande + nytillagda sources
  8. poolRoundNumber += 1
  9. Upprepa tills alla sources lämnat poolen eller nått 3 rundor
```
