#!/usr/bin/env python3
"""
scb-500-AI.py — Sök varje 500-källa i postTestC-error500 med Claude Code (MiniMax M2.7 via ollama),
retry ×3 med exponential backoff (30s/60s/120s), öppnar separat terminal per källa,
verifiera live-URL, skriver .md till RawSources (500-agg-*),
och flyttar källan till preA (lyckad) eller postTestC-404 (misslyckad).
...
Flow per sourceId:
  ...
  3. Om retries misslyckas: Claude-anrop → sök ny URL
  4. HTTP-verify + scoring (threshold 1.0)
  5. Spara .md i RawSources/500-agg-*.md
  6. Lyckad → preA, misslyckad → postTestC-404
...
RUN_LOG       = LOGS_DIR / f"scb-500-AI-{datetime.now().isoformat().replace(':', '-').replace('.', '-')}.log"
...
def retry_with_backoff(url: str, source_id: str) -> tuple[bool, int, str, int]:
    ...
    for attempt_idx, delay in enumerate(RETRY_DELAYS):
        ...
        if not ok:
            log(f"    [retry-{attempt_idx+1}] connection error")
        ...
        # Other error (404, 401, 403, etc.) — not retryable
...
def update_source_truth(source_id: str, verified_url: str, status_code: int,
    ...
    try:
        ...
        source["metadata"][recovery_key] = {
            ...
            "recoveredBy": f"scb-500-AI.py ({found_via})",
...
def write_md(source_id: str, verified_url: str, status_code: int,
            ...
            score_log: str,
            found_via: str = "claude-ai-search",
...
_genererad: {ts} av scb-500-AI.py_
"""
    out_path.write_text(content, encoding="utf-8")
    return out_path

# ── Claude Code prompt template ────────────────────────────────────────────────
PROMPT_TEMPLATE = """Du ska hitta rätt event-URL för en svensk venue som tidigare fått HTTP 500 (serverfel).
Källan har RETRY:ATS utan framgång (30s → 60s → 120s backoff).
Nu söker du en ny fungerande URL via webbsökning.

Källa: {source_id}
Venue-namn: {venue_name}
Ursprunglig domän (som gav 500): https://{orig_domain}
...
6. Skriv en .md-fil till {raw_sources}/500-agg-{{source_id}}-{{domän}}.md med frontmatter:
---
originalSourceId: {{source_id}}
...
**Original (500-fel):** `https://{{orig_domain}}`
...
def run_claude_for_source(source_id: str, venue_name: str, retry_attempts: int = 3) -> Path | None:
    ...
    prompt = PROMPT_TEMPLATE.format(
        ...
        out_q=str(FAIL_Q),  # Will be overridden in main for success case
...
def main():
    ...
    log(f"scb-500-AI (Claude Code MiniMax/ollama)  │  {total} källor i postTestC-error500")
    log(f"  modell: {OLLAMA_MODEL} @ {OLLAMA_BASE_URL}")
    log(f"  RETRY: 30s → 60s → 120s  │  falls back till Claude-sök")
    ...
    for idx, entry in enumerate(entries):
        ...
        # ── STEP 2: Retry failed — try Claude AI search ──────────────────────
        log(f"  [search] alla {len(RETRY_DELAYS)} retries misslyckades, Claude söker...")
        ...
        result = wait_for_marker(done_marker, timeout=180)
        log_file = PROMPT_DIR / f"log-{source_id}.txt"
        ...
        if result.startswith("DONE:"):
            ...
            md_path = RAW_SOURCES / f"500-agg-{source_id}-{dom_slug}.md"
            ...
            log(f"  ✓ Claude: {verified_url} (HTTP {status_code}, score={combined_score:.1f})")
            ...
            entry_out["foundVia"] = "claude-ai-search"
    ...
    log("")
    log(f"KLAR  │  {recovered_via_retry} retry-fixade  │  {recovered_via_search} claude-fixade  │  {failed} misslyckades  │  {len(remaining)} kvar")
