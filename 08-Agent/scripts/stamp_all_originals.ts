/**
 * 08-Agent/scripts/stamp_all_originals.ts
 *
 * Bygger `import-stamped/` som ett DERIVAT av `import-original/`.
 *
 * Den centrala regeln
 * -------------------
 * Scriptet läser ALLTID från `import-original/` och skriver ALLTID till
 * `import-stamped/`. Det läser aldrig sin egen output. Därför kan stämpelns
 ...
 * Bakgrund (2026-09-01)
 * ---------------------
 * Två-roll-bibliotek infört:
 *   - import-original/  ← BFL-original, ostämplade (råa)
 *   - import-stamped/   ← samma bilder med transparent AI Act-stämpel
 *
 * Detta script ersätter det tidigare `build_ai_stamped.ts` som opererade
 * på `ai-originals/` + `ai-stamped/`. Bucket-strukturen är nu två-roll-
 * tydlig: vem som helst som ser `import-original/` vet att den INTE ska
 * serveras till UI.
 *
 * Position
 * --------
 * Default är `bottom-left`: stämpeln läggs i nedre vänstra hörnet (24 px
 * inset, 200×48 px på 1024×1024-bild). EU AI Act Art. 50 disclosure.
 *
 * `--position=bottom-right` ger stämpeln i högra hörnet istället.
 *
 * Kostnad: 0 kr. Sharp-compute lokalt, ingen modell-API anropas.
 *
 * Idempotent: filer som redan finns i `import-stamped/` hoppas över såvida
 * inte `--force`. Med `--force` byggs de om från originalet — inte från
 * det befintliga derivatet.
 *
 * Användning:
 *   npx tsx --env-file=.env 08-Agent/scripts/stamp_all_originals.ts [--limit=N] [--dry-run] [--force] [--position=bottom-left]
 */
...
const SOURCE_PREFIX = 'import-original';
const TARGET_PREFIX = 'import-stamped';
...
async function main(): Promise<void> {
  ...
  console.log(
    `[stamp-all-originals] ${SOURCE_PREFIX}/ → ${TARGET_PREFIX}/  ` +
  ...
  console.log(`[stamp-all-originals] källa=${sourceNames.length}  redan i mål=${existing.size}`);

  let candidates = sourceNames;
  if (Number.isFinite(args.limit) && candidates.length > args.limit) {
    console.log(`[stamp-all-originals] --limit: tar första ${args.limit} av ${candidates.length}`);
  ...
  console.log('');
  console.log(`[stamp-all-originals] DONE in ${elapsed}s`);
...
main().catch((err: unknown) => {
  const msg = err instanceof Error ? err.message : String(err);
  console.error('[stamp-all-originals] FATAL:', msg);
  process.exit(1);
});