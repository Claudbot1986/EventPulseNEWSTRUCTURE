...
- `20260920-0001-user-preferences-auth-uid.sql` — realigns `user_preferences` with anonymous sign-ins: `client_user_id` TEXT→UUID, FK→`auth.users` ON DELETE CASCADE (NOT VALID; pre-auth device rows kept as unlinked legacy), owner-only RLS for `authenticated` replaces the open/block-all pre-auth policy mix.
