...
export default function App({ onUserLoggedOut, onOpenLogin }) {
  ...
  //
  // Auth callback (`eventpulse://auth/callback?...`) is owned by AppShell
  // — bail out before the share-parser sees it so we never try to call
  // `/s/<token_hash>` against a magic-link URL.
  useEffect(() => {
    let cancelled = false;
    const resolveDeepLink = async (url) => {
      if (cancelled) return;
      // Auth callbacks are owned by AppShell (MagicLinkHandlerScreen) —
