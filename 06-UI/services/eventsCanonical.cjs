/**
 * Canonical event read path — single source for wrapper (:7777) and UI client.
...
function transformEvent(row) {
  ...
  const description = row.description_sv || row.description_en || '';
...
function buildFilterQuery({ days = 365, source = null }) {
  ...
  params.set('start_time', `gte.${now.toISOString()}`);
  params.append('start_time', `lte.${end.toISOString()}`);
...
/**
 * Canonical bundle — same shape for wrapper and app.
 */
async function fetchCanonicalEvents(options = {}) {
  ...
  return {
    ...
    events,
    count: events.length,
    query_params: { city, days, limit, offset, source },
    metadata: {
      data_source: 'supabase',
