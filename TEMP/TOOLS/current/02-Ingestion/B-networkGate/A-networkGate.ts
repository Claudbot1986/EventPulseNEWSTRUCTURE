...
export async function evaluateNetworkGate(
  ...
  if (phaseMode === 1) {
    ...
    return {
      nextPath: hasSignals ? 'network' : 'html',
      reason: hasSignals
        ? `[sanity] Network signals found — routing to Network Path`
        : `[sanity] No network signals — routing to HTML`,
      modelStatus: 'prelim_1src',
      inspectorResult,
      networkSignalsFound: hasSignals,
      openEventDataAccessible: hasSignals ? accessible : false,
      phaseMode,
    };
  }
  ...
  if (phaseMode === 2) {
    ...
    if (accessible) {
      return {
        nextPath: 'network',
        reason: `[breadth] Open event data accessible — routing to Network Path: ${accessReason}`,
        modelStatus: 'prelim_1src',
        inspectorResult,
    ...
    return {
      nextPath: 'html',
      reason: `[breadth] Network signals found but not open/accessible — routing to HTML: ${accessReason}. GotEvent lesson: API structure ≠ usable event data.`,
      modelStatus: 'prelim_1src',
      inspectorResult,
      networkSignalsFound: true,
      openEventDataAccessible: false,
  ...
  if (accessible) {
    ...
    if (likely.length > 0) {
      return {
        nextPath: 'network',
        reason: `[smoke] Likely event API confirmed — routing to Network Path`,
        modelStatus: 'prelim_1src',
        inspectorResult,
        networkSignalsFound: true,
        openEventDataAccessible: true,
  ...
  return {
    nextPath: 'html',
    reason: `[smoke] No confirmed event API — routing to HTML`,
    modelStatus: 'prelim_1src',
    inspectorResult,
